...
df=pd.read_csv('solar+all_label copy.csv')

state=df["State"]
name=df["Name"]
df.drop(["State","Name","Surface_Albedo","Precipitable_Water","Wind_Speed"], axis=1, inplace=True) #could also leave in other variables 
# save=df.iloc[:,:4]  #if I do want to normalize, this is to reinsert later but i deleted that line (df.iloc[:,:4]=save)
# df.iloc[:,:4]=normalize(df.iloc[:,:4],axis=0)  #I dont think decition trees need normalization
...
count=0
params={
    'n_estimators': 900, 'min_samples_split': 3, 'min_samples_leaf': 1, 'max_leaf_nodes': 30, 'criterion': 'gini','max_depth':5 #5 or 7 max depth
}
while(count<9):  
...
def plot_iterations_scores(f1_scores, accuracy_scores):
    ...
    plt.ylabel("Score")
    plt.title("F1-Score and Accuracy vs. Iterations for the Solar RFC Model")
    plt.ylim(0.85, 1)
    plt.grid(True)
    plt.legend()
...
#     plt.figure(figsize=(20, 8))
    # plot_tree(model.estimators_[1], feature_names=["Temperature",	"GHI",	"Relative_Humidity",	"Pressure"],class_names=['Not Suitable', 'Suitable'], filled=True)
#     plt.show()
# for i in range (0,5):
#     show_tree(model,i)
...
df.to_csv("All_labels_Solar.csv",index=False)
